% ======================================================================
%  plot_aperiodic_brain_region_v3.m
%
%  Same as plot_aperiodic_brain_region_v2.m, extended to also plot/test
%  the two new predictors added in ap_reward_glm_analysis_v2.m:
%    'initialbar'  (InitialBarZ - z-scored per session)
%    'playedtr'    (PlayedTrC - centered by 2.5)
%  in addition to the existing 'avgpupil'.
%
%  No other logic changes - gather_values/group_ttest_str/
%  make_scatter_plot/make_histogram_plot already generalize over
%  whatever is in the `predictors` list, so this just adds two more
%  entries and everything downstream (scatter dots, histograms,
%  t-tests, figure saving) runs for all three automatically.
%
...
%  channelToBrainRegion.m / ap_reward_glm_analysis_v2.m), so this script
...
%    predictor  = {'avgpupil', 'initialbar', 'playedtr'}
%  TWO figures:
...
% ---------------------------- CONFIG ---------------------------------
M_FILE = "Z:\users\Jeremiah\M\M_pupil_playedtr_initialbar_glm_results.mat";
N_FILE = "Z:\users\Jeremiah\N\N_pupil_playedtr_initialbar_glm_results.mat";
...
% (see ap_reward_glm_analysis_v2.m's save(...,'M') / save(...,'N') calls),
...
predictors = {'avgpupil', 'initialbar', 'playedtr'};
...
% text color used for figure-level titles/labels that sit directly on
% the figure background (as opposed to axes titles, which sit on the
% default-white axes background regardless of FIG_BG_COLOR) - chosen
% so it stays legible against dark backgrounds like 'k'.
if isequal(FIG_BG_COLOR, 'k') || isequal(FIG_BG_COLOR, [0 0 0])
    FIG_TEXT_COLOR = 'w';
else
    FIG_TEXT_COLOR = 'k';
end
...
for g = 1:numel(monkeyGroups)
    ...
    for p = 1:numel(parameters)
        ...
        for r = 1:numel(predictors)
            ...
            % fh1 = make_scatter_plot(areaData, ALPHA, [base_title ' scatter'], FIG_BG_COLOR, FIG_TEXT_COLOR);
            % figHandles(end+1) = fh1; %#ok<AGROW>

            fh2 = make_histogram_plot(areaData, ALPHA, [base_title ' histogram'], FIG_BG_COLOR, FIG_TEXT_COLOR);
...
% ======================================================================
%  bin_edges_safe - linspace(min,max,nBins+1), but falls back to a
%  single valid bin centered on the (identical) value when all input
%  values are equal (zero variance), since linspace collapses to
%  repeated edges in that case and histogram() requires strictly
%  increasing edges.
% ======================================================================
function edges = bin_edges_safe(vals, nBins)
    lo = min(vals);
    hi = max(vals);
    if lo == hi
        edges = lo + (-0.5:1:0.5);
...
% ======================================================================
function fh = make_scatter_plot(areaData, alpha, plot_title, bgColor, textColor)
    ...
    title({strrep(plot_title, '_', ' '), strjoin(statsStrs, ' | ')}, 'Color', textColor);
...
% ======================================================================
function fh = make_histogram_plot(areaData, alpha, plot_title, bgColor, textColor)
    ...
    title(tl, strrep(plot_title, '_', ' '), 'Color', textColor);

    if isempty(allVals)
        nexttile(tl);
        title('No data', 'Color', textColor);
        return
    end

    nBins = 20;
    edges = bin_edges_safe(allVals, nBins);
    ...
    for a = 1:nAreas
        ...
        if isempty(coefVec)
            title(ax, sprintf('%s (n=0)', areaData(a).name), 'Color', textColor);
            ax.XColor = textColor;
            ax.YColor = textColor;
            hold(ax, 'off');
            continue
        ...
        title(ax, sprintf('%s (n=%d, %.1f%% sig)\n%s', areaData(a).name, nA, pctSig, ttestStr), 'Color', textColor);
